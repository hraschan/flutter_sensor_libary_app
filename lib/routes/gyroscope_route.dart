...
import 'package:sensor_library/models/raw_sensors/gyroscope.dart';
...
class _GyroscopeRouteState extends State<GyroscopeRoute> {
  ...
  void initState() {
    ...
    gyro = Gyroscope(inMillis: 500);
    gyro.getRaw().listen((element) {
      if (mounted) {
        setState(() {
          if (element.x >= 0 && element.x <= 1) _x = element.x;
          _gyroX = element.x;

          if (element.y >= 0 && element.y <= 1) _y = pi * element.y;
          _gyroY = element.y;
